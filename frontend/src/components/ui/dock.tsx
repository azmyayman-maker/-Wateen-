...
type DockProps = { children: React.ReactNode; className?: string; distance?: number; panelHeight?: number; magnification?: number; spring?: SpringOptions; };
type DockItemProps = { className?: string; children: React.ReactNode; onClick?: () => void; };
...
function Dock({ children, className, spring = { mass: 0.1, stiffness: 150, damping: 12 }, magnification = DEFAULT_MAGNIFICATION, distance = DEFAULT_DISTANCE, panelHeight = DEFAULT_PANEL_HEIGHT }: DockProps) {
  ...
  return (
    <motion.div style={{ height: height, scrollbarWidth: 'none' }} className='mx-2 flex max-w-full items-end overflow-x-auto z-50'>
      <motion.div
        onMouseMove={({ pageX }) => { isHovered.set(1); mouseX.set(pageX); }}
        onMouseLeave={() => { isHovered.set(0); mouseX.set(Infinity); }}
        className={cn('mx-auto flex w-fit gap-4 rounded-2xl bg-gray-50/10 backdrop-blur-md border border-white/10 px-4 dark:bg-neutral-900/50 shadow-2xl', className)}
...
function DockItem({ children, className, onClick }: DockItemProps) {
  ...
  const mouseDistance = useTransform(mouseX, (val) => {
    ...
    // In RTL, the mouseX calculation naturally mirrors if we consider pageX from left. 
    // We leave this logic as-is, typically Framer handles simple pageX transforms fine, 
    // but the actual visual might flip depending on how the browser scales bounds.
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn('absolute -top-10 left-1/2 w-fit whitespace-pre rounded-md border border-white/10 bg-[#0A0A1A]/90 backdrop-blur-md px-3 py-1.5 text-xs text-white shadow-xl font-bold font-cairo z-50', className)}
          style={{ x: '-50%' }}
        >
          {children}
        </motion.div>
...
function DockIcon({ children, className, ...rest }: DockIconProps) {
  ...
  return <motion.div style={{ width: widthTransform }} className={cn('flex items-center justify-center text-neutral-300 hover:text-[#dd7bbb] transition-colors', className)}>{children}</motion.div>;
}
