...
from users.models import AgencyProfile, AgencyStatus, DispatchMode, NurseProfile
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS('Created "Wateen Internal Agency".')
                )
            else:
            ...
            if count == 0:
                self.stdout.write(
                    self.style.SUCCESS("No orphaned nurses found. Migration complete.")
                )
                return

            self.stdout.write(
                self.style.NOTICE(
                    f"Found {count} orphaned nurses. Assigning to Internal Agency..."
                )
            )
            ...
            internal_agency.network_capacity = NurseProfile.objects.filter(
                agency=internal_agency
            ).count()
            internal_agency.save(update_fields=["network_capacity"])

            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully migrated {updated_count} nurses and updated network capacity."
                )
            )
