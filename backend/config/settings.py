...
if os.name == "nt" and not os.environ.get("GDAL_LIBRARY_PATH"):
    try:
        ...
        _GDAL_WARNING = (
            "\n"
            "======================================================================\n"
            "[WARN] GDAL NOT FOUND — Mocking GeoDjango for local execution\n"
            "======================================================================\n"
        )
        print(_GDAL_WARNING, file=sys.stderr)

if not _GDAL_AVAILABLE:
    GDAL_LIBRARY_PATH = "mock_gdal.dll"
    GEOS_LIBRARY_PATH = "mock_geos.dll"

    # Patch ctypes to return mock DLL objects to prevent load errors on Windows
    import ctypes
            ...
            if attr not in self._funcs:
                ...
                class MockFunc:
                    ...
                    def __call__(self, *args, **kwargs):
                        if self._name == 'GDALVersionInfo':
                            return b'3.0.0'
                        elif self._name == 'GEOSversion':
                            return b'3.8.0-CAPI-1.13.1'
                        
                        if self._name == 'GEOSSetSRID_r':
                            if len(args) >= 3:
                                geom = args[1]
                                srid = args[2]
                                geom_addr = getattr(geom, 'value', None) or id(geom)
                                cdll_self._srids[geom_addr] = srid
                            return 1
                        elif self._name == 'GEOSGetSRID_r':
                            if len(args) >= 2:
                                geom = args[1]
                                geom_addr = getattr(geom, 'value', None) or id(geom)
                                return cdll_self._srids.get(geom_addr, 0)
                            return 0

                        if self.restype:
                            try:
                                if issubclass(self.restype, ctypes.c_char_p):
                                    return b'POINT (1.0 2.0)'
                                elif issubclass(self.restype, (ctypes.c_double, ctypes.c_float)):
                                    return 1.0
                                elif issubclass(self.restype, (ctypes.c_int, ctypes.c_uint, ctypes.c_long, ctypes.c_ulong, ctypes.c_short, ctypes.c_ushort, ctypes.c_byte, ctypes.c_ubyte, ctypes.c_char)):
                                    return 1
                                return ctypes.cast(1, self.restype)
                            except Exception:
                                try:
                                    return ctypes.cast(1, self.restype)
                                except Exception:
