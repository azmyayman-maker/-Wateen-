...
class AgencyDashboardOverviewView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # 1. Real-time stats
        stats = {
            "pending_visits": Visit.objects.filter(agency_id=agency_id, status=VisitStatus.PENDING_AGENCY).count(),
            "active_visits": Visit.objects.filter(agency_id=agency_id, status__in=[VisitStatus.PENDING_NURSE, VisitStatus.ACCEPTED, VisitStatus.EN_ROUTE, VisitStatus.IN_PROGRESS]).count(),
            "completed_today": Visit.objects.filter(agency_id=agency_id, status=VisitStatus.COMPLETED, updated_at__date=timezone.now().date()).count(),
            "online_nurses": NurseProfile.objects.filter(agency_id=agency_id, is_available=True).count(),
            "total_nurses": NurseProfile.objects.filter(agency_id=agency_id).count(),
        }
        
        # 2. Financial summary (Mocked for now)
        financials = {
