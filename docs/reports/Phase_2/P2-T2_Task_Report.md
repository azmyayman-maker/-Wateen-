...
الميزة جاهزة إلى حد كبير مع أعمال تصحيح جارية. اختباران من أصل 16 (test_kyc_queue_list_pending_only و test_kyc_queue_oldest_first) يفشلان حالياً بسبب خلل في إعداد بيانات الاختبار (Fixture) حيث لا يتم ربط المستخدم بالوكالة بشكل صحيح. يجري إصلاح هذا الخلل حالياً.
